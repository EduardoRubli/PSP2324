...
// Actividad8d (4 hilos y contenedor con límite de 100).
        ...
        try {
            while (!Thread.currentThread().isInterrupted()) {
                synchronized (contenedor) {
                    // Esperar si se alcanzan las 100 cápsulas.
                    while (contenedor.size() >= 100) {
                        System.out.println("Contenedor lleno...");
                        contenedor.wait(); // Esperar a que se consuman cápsulas.
                    }
                    ...
                    // Notificar al hilo consumidor si se alcanzan las 100 cápsulas.
                    if (contenedor.size() == 100) {
...
class Consumidor implements Runnable {
    ...
    public void run() {
        try {
            while (!Thread.currentThread().isInterrupted()) {
                synchronized (contenedor) {
                    // Esperar mientras el contenedor esté vacío.
                    ...
                    // Notificar al productor si hay menos de 100 cápsulas.
                    if (contenedor.size() < 100) {
                        contenedor.notify();
                    }
                }
